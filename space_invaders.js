...
function draw() {
  ...
  // Aggiorna la posizione del giocatore con accelerazione e attrito
  playerVelocity *= playerFriction;
  player.x += playerVelocity;
  
  // Limita la posizione del giocatore
  player.x = constrain(player.x, player.width/2, width - player.width/2);
  
  // Disegna e aggiorna il giocatore
  drawPlayer();
  
  // Disegna e aggiorna gli invasori
  updateInvaders();
  
  // Disegna e aggiorna i proiettili
  updateBullets();
  
  // Disegna e aggiorna i proiettili degli invasori
...
function showStartScreen() {
  ...
  textSize(24);
  text("Premi SPACE per iniziare", width/2, height/2 + 50);
}
...
function showGameOver() {
  ...
  text("Premi SPACE per rigiocare", width/2, height/2 + 50);
}
...
function drawInvader(invader) {
  ...
  switch(invader.type) {
    case 0: // Primo tipo (rossi) - Granchio
      // Corpo principale
    ...
    case 1: // Secondo tipo (arancioni) - Polpo
      // Corpo principale
    ...
    case 2: // Terzo tipo (gialli) - Medusa
      // Corpo principale
      beginShape();
    ...
    case 3: // Quarto tipo (verdi) - Ragno
      // Corpo principale
    ...
    case 4: // Quinto tipo (blu) - Calamaro
      // Corpo principale
      beginShape();
...
function updateInvaders() {
  ...
  // Controlla se gli invasori hanno raggiunto i bordi
  ...
  for (let i = invaders.length - 1; i >= 0; i--) {
    ...
    // Disegna l'invasore con la nuova sprite
    drawInvader(invader);
    
    // Gli invasori sparano casualmente
...
function playEnemyExplosion(type) {
  ...
  // Suoni diversi per ogni tipo di invasore
  switch(type) {
    case 0: // Granchio
...
function updateBullets() {
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    // Controlla collisioni con gli scudi
...
function drawShields() {
  for (let shield of shields) {
    if (shield.health > 0) {
      ...
      // Disegna la base della barriera
      fill(shield.color);
      noStroke();
      ...
      // Aggiungi dettagli alla barriera
...
function keyPressed() {
  if (!gameStarted && keyCode === 32) { // SPACE
    gameStarted = true;
    return;
  }
  
  if (gameOver && keyCode === 32) { // SPACE
    resetGame();
    return;
  }
